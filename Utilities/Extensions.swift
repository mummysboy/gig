...
extension View {
    func debugFrame(_ label: String = "") -> some View {
        self.onAppear {
            print("[DEBUG] Frame for \(label): \(self)")
        }
    }
    
    func debugImageURL(_ url: String?, label: String = "") -> some View {
        self.onAppear {
            if let url = url {
                print("[DEBUG] Image URL for \(label): \(url)")
                if !url.isValidURL {
                    print("[DEBUG] ⚠️ Invalid URL detected for \(label): \(url)")
                }
            } else {
                print("[DEBUG] ⚠️ Nil URL for \(label)")
            }
        }
    }
}

// MARK: - Safe AsyncImage Wrapper
    ...
    var body: some View {
        if let imageURL = url,
           ...
           let safeURL = imageURL.safeURL {
            ...
            .onAppear {
                print("[SafeAsyncImage] Loading image from: \(safeURL)")
            }
        } else {
            placeholder()
                .onAppear {
                    print("[SafeAsyncImage] Using fallback. URL was: \(url ?? "nil")")
                }
        }
    }
} 
